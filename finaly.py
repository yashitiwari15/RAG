...
def extract_text_by_page_with_metadata(pdf_file):
    page_texts = []
    patient_name = None  # Reset patient name for each new PDF file
    try:
        ...
        for page_number in range(len(reader.pages)):
            ...
            if text:
                # Try to detect patient name on each page until found
                if not patient_name:
...
if pdf_files:
    try:
        ...
        for pdf_name, page_texts in page_data:
            ...
            if len(chunks) == 0:
                st.error(f"No chunks found in {pdf_name}. Skipping this file.")
                continue
        ...
        for embeddings, chunks, chunk_page_numbers, pdf_name, hierarchical_structure in all_embeddings:
            ...
            if num_embeddings != len(chunks):
                st.error(f"Mismatch in embeddings and chunks length: {num_embeddings} vs {len(chunks)}")
                continue
            ...
            for level, summaries in hierarchical_structure.items():
                if isinstance(level, str) and "level_" in level:
                    ...
                    for summary in summaries:
                        ...
                        metadata.append({"pdf_name": pdf_name, "chunk": summary, "page_number": None, "patient_name": patient_names[0], "chunk_index": -1, "hierarchical_level": level_num})
