...
def retrieve_and_rank_context(query_embedding, top_k=5):
    if not collection:
        return []
    ...
    ranked_results = sorted(results[0], key=lambda x: x.distance)
    return ranked_results

def create_context_from_ranked_metadata(ranked_results, max_tokens=1000):
    context_chunks = []
    context_metadata = []
    ...
    for hit in ranked_results:
        ...
        if token_count + tokens_in_chunk <= max_tokens:
...
You are given 10 metadata contaning feilds like chunk and patient name. Answer the question by corretly matching the name in query and patient_name in metadata only.Name matching is must
...
if query:
    ...
    ranked_results = retrieve_and_rank_context(query_embedding)
    st.write("Going to retrieve documents")

    if ranked_results:
        with st.spinner("Assistant is typing..."):
            context, context_metadata = create_context_from_ranked_metadata(ranked_results)
